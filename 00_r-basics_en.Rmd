---
title: "[EN] Introduction to R and `RStudio`"
...
| Basic functions: `head()`, `str()`, `summary()` |  | Create basic visualizations with `ggplot2` |
...
3.  Create a histogram for the `plant_height` variable:

    ```{r eval=FALSE}
    ggplot(tapak_dara, aes(x = plant_height)) +
...
1.  Create a boxplot of `plant_height` by location (`label`):

    ```{r eval=FALSE}
    ggplot(tapak_dara, aes(x = label, y = plant_height, fill = label)) +
      geom_boxplot() +
